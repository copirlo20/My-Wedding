...
(function (we, $e) {
	...
	const Xo = /\B([A-Z])/g,
		...
		Jo = /-(\w)/g,
		xe = rt((t) => (t.charAt(0).toLowerCase() + t.slice(1)).replace(Jo, (e, i) => i.toUpperCase())),
	...
	function Gi(t) {
		return ((t[0] = Yi(t[0])), N(t[1]) && (t[1] = t[1].split(' ')), et(t[2]) && t.splice(2, 0, !1), t);
	}
				...
				(o) =>
					new Promise((r, a) => {
						...
						(R(
							...
							ut(e, Ae) && (s && b(o, `uk-transform-origin-${s}`), n && b(o, `${Ae}reverse`)));
					})
			)
		);
	}
	...
	function ts(t) {
		...
		return {
			top: e - k(c(t, 'marginTop')),
			left: i - k(c(t, 'marginLeft'))
		};
	}
	...
	function Rt(t) {
		...
		for (; (t = t.offsetParent); )
			if (((e[0] += t.offsetTop + k(c(t, 'borderTopWidth'))), (e[1] += t.offsetLeft + k(c(t, 'borderLeftWidth'))), c(t, 'position') === 'fixed')) {
				const i = ie(t);
	...
	function es(t) {
		(Js(X.reads), Js(X.writes.splice(0)), (X.scheduled = !1), (X.reads.length || X.writes.length) && is(t + 1));
	}
	const yr = 4;

	function is(t) {
		X.scheduled || ((X.scheduled = !0), t && t < yr ? Promise.resolve().then(() => es(t)) : requestAnimationFrame(() => es(1)));
	}
	...
	function _r(t, e, { offset: i, attach: s }, n) {
		return dn(t, e, {
			attach: {
				element: vn(s.element, n),
				target: vn(s.target, n)
			},
			offset: Ar(i, n)
		});
	}
	...
	var Br = {
		...
		connected() {
			(p(this.$el, 'role', this.role), (this.date = k(Date.parse(this.$props.date))), (this.end = !1), this.start());
		},
		disconnected() {
			this.stop();
		},
	...
	const V = {};
	((V.events = V.watch = V.observe = V.created = V.beforeConnect = V.connected = V.beforeDisconnect = V.disconnected = V.destroy = fs),
		(V.args = function (t, e) {
			return e !== !1 && fs(e || t);
		}),
		...
		return {
			...
			handler(e) {
				...
				R(document, `${At} ${fi} scroll`, (n) => {
					const { x: o, y: r } = re(n);
					((n.type !== 'scroll' && s && o && Math.abs(i.x - o) > 100) || (r && Math.abs(i.y - r) > 100)) &&
						setTimeout(() => {
	...
	function ia(t, e, i) {
		return [...t].sort(
			(s, n) =>
				Y(s, e).localeCompare(Y(n, e), void 0, {
					numeric: !0
				}) * (i === 'asc' || -1)
		);
	}
	...
	var Ut = {
		...
		computed: {
			...
			hasTransition: ({ animation: t }) => ['slide', 'reveal'].some((e) => ut(t[0], e))
		},
		methods: {
					...
					return (
						await Promise.all(
							T(t).map((s) => {
								...
								const o = (et(i) ? i : i === !1 || !this.hasAnimation ? na : this.hasTransition ? oa : ra)(s, n, this),
			...
			isToggled(t = this.$el) {
				return ((t = F(t)), I(t, this.clsEnter) ? !0 : I(t, this.clsLeave) ? !1 : this.cls ? I(t, this.cls.split(' ')[0]) : W(t));
			},
			_toggle(t, e) {
				if (!t) return;
				e = !!e;
				let i;
				(this.cls ? ((i = m(this.cls, ' ') || e !== I(t, this.cls)), i && j(t, this.cls, m(this.cls, ' ') ? void 0 : e)) : ((i = e === t.hidden), i && (t.hidden = !e)), z('[autofocus]', t).some((s) => (W(s) ? s.focus() || !0 : s.blur())), i && v(t, 'toggled', [e, this]));
			}
		}
	};
	...
	async function oa(t, e, { animation: i, duration: s, velocity: n, transition: o, _toggle: r }) {
		...
		const Mi = Object.fromEntries(['padding', 'border', 'width', 'height', 'minWidth', 'minHeight', 'overflowY', 'overflowX', C, D].map((Yo) => [Yo, t.style[Yo]])),
			ft = x(t),
			be = k(c(t, C)),
			Uo = k(c(t, D)),
			Zt = ft[y] + Uo;
		...
		(c(Di, {
			...
			height: ft.height,
			width: ft.width,
			...c(t, ['overflow', 'padding', 'borderTop', 'borderRight', 'borderBottom', 'borderLeft', 'borderImage', D])
		}),
			c(t, {
				padding: 0,
				border: 0,
	...
	var xs = {
		...
		methods: {
			...
			show() {
				return this.container && E(this.$el) !== this.container ? (H(this.container, this.$el), new Promise((t) => requestAnimationFrame(() => this.show().then(t)))) : this.toggleElement(this.$el, !0, An);
			},
			hide() {
		...
		return {
			...
			show(u, f = 0, d) {
				...
				return (
					...
					new Promise((y) => {
						(h || (h = y),
							Promise.all([M.start(e, l[1], u, g), M.start(t, l[0], u, g)]).then(() => {
								(this.reset(), h());
							}, _));
					})
				);
			...
			translate(u) {
				...
				(c(e, f[1]),
					c(t, f[0]),
					bi(e, 'itemtranslatein', {
						percent: u,
						dir: i
					}),
					bi(t, 'itemtranslateout', {
						percent: 1 - u,
						dir: i
					}));
			},
	...
	var pa = {
		...
		methods: {
			...
			move(t) {
				...
				if ((zt(document, Ss, this.move, ks), zt(document, Dn, this.end, Mn), this.dragging))
	...
	function va(t, e) {
		for (const { name: i, handler: s, immediate: n = !0 } of t._watches) ((t._initial && n) || (dt(e, i) && !ti(e[i], t[i]))) && s.call(t, t[i], e[i]);
		t._initial = !1;
	}
	...
	function Fn(t, e, i) {
		...
		((n = et(n) ? n.call(t, t) : n || t.$el),
			!(!n || (K(n) && !n.length) || (h && !h.call(t))) &&
				t._events.push(
					$(n, s, l ? (N(l) ? l : l.call(t, t)) : null, N(o) ? t[o] : o.bind(t), {
						passive: a,
						capture: r,
						self: u
					})
	...
	function Wn(t) {
		...
		for (const a in s) {
			const l = Mt(a);
			let h = Y(n, l);
			G(h) || ((h = s[a] === Boolean && h === '' ? !0 : ds(s[a], h)), !(l === 'target' && ut(h, '_')) && (e[a] = h));
		}
		const r = fe(Y(n, o), i);
		for (const a in r) {
			const l = xe(a);
	...
	function Cs(t) {
		t._connected || (Ta(t), ge(t, 'beforeConnect'), (t._connected = !0), ya(t), Lr(t), ma(t), Sa(t), Pa(t), wa(t), ge(t, 'connected'), He(t));
	}

	function Ts(t) {
		t._connected && (ge(t, 'beforeDisconnect'), ka(t), Rr(t), Ia(t), $a(t), ge(t, 'disconnected'), (t._connected = !1));
	}
	let _a = 0;
	...
	var Yn = {
			...
			scale: {
				...
				translate(t) {
					return [
						{
							opacity: 1 - t,
							transform: pe(1 - 0.2 * t)
						},
						{
							opacity: t,
							transform: pe(1 - 0.2 + 0.2 * t)
						}
					];
				}
			}
		...
		Jn = {
			...
			created() {
				...
				(i &&
					s &&
					(i.dataset.i18n = JSON.stringify({
						label: s
					})),
					this.$mount(H(this.container, t)));
			},
			computed: {
				caption: ({ selCaption: t }, e) => w(t, e)
			},
			events: [
				{
					name: `${ui} ${gt} keydown`,
				...
				},
				{
					name: 'beforeitemshow',
					handler(t) {
						this.isToggled() || ((this.draggable = !1), t.preventDefault(), this.toggleElement(this.$el, !0, !1), (this.animation = Xn.scale), O(t.target, this.clsActive), this.stack.splice(1, 0, this.index));
					}
				},
				{
					name: 'itemshow',
	...
	var xi = {
		...
		connected() {
			...
			if (((this.matchMedia = !0), t)) {
				...
				const e = () => {
					((this.matchMedia = this.mediaObj.matches), v(this.$el, Ft('mediachange', !1, !0, [this.mediaObj])));
				};
	...
	function ki(t, e, i) {
		...
		return (
			t === 'x' || t === 'y'
				? ((t = `translate${Ct(t)}`), (n = (o) => k(k(o).toFixed(s === 'px' ? 0 : 6))))
				: t === 'scale' &&
					((s = ''),
					(n = (o) => {
						var r;
						return Ii([o]) ? Q(o, 'width', e, !0) / e[`offset${(r = o.endsWith) != null && r.call(o, 'vh') ? 'Height' : 'Width'}`] : k(o);
					})),
	...
	function Es(t, e, i) {
		return (
			...
			(s, n) => {
				const [o, r, a] = no(i, n),
					l = o.map((h, u) => ((h += a * (r[u] - h)), u === 3 ? k(h) : parseInt(h, 10))).join(',');
				s[t] = `rgba(${l})`;
			}
		);
	}
	...
	var Qa = {
		...
		computed: {
			...
			start({ start: t }) {
				...
				return Q(t || ((e = (1 - e) * 100) && `${e}vh+${e}%`), 'height', this.target, !0);
			}
		},
		observe: [
			gs(),
			...
			translate(h) {
				...
				for (const y of B(o)) {
					const C = m(f, y),
						D = y === d,
						wt = y === g,
						Ke = D || (!wt && (C || (i * (U ? -1 : 1) === -1) ^ (Ti(y, o) > Ti(t || e))));
		...
		methods: {
			...
			updateActiveClasses() {
				const t = this._getTransitioner(this.index).getActives(),
					e = [this.clsActive, ((!this.sets || m(this.sets, k(this.index))) && this.clsActivated) || ''];
	...
	var bo = {
		...
		methods: {
			...
			getPositionOffset(t) {
				return Q(this.offset === !1 ? c(t, '--uk-position-offset') : this.offset, this.axis === 'x' ? 'width' : 'height', t) * (m(['left', 'top'], this.dir) ? -1 : 1) * (this.inset ? -1 : 1);
			},
			getShiftOffset(t) {
	...
	var ml = {
		...
		methods: {
			...
			async hide() {
				var t;
				A(this.$el, 'input:focus') || (clearTimeout(this.showTimer), this.isToggled(this.tooltip || null) && (await this.toggleElement(this.tooltip, !1, !1)), (t = this.reset) == null || t.call(this), at(this.tooltip), (this.tooltip = null));
			},
			async _show(t, e) {
				((this.tooltip = H(this.container, `<div id="${e}" class="uk-${this.$options.name}" role="tooltip"> <div class="uk-${this.$options.name}-inner">${t}</div> </div>`)),
					$(this.tooltip, 'toggled', (i, s) => {
						...
						const a = [
							R(document, `keydown ${gt}`, this.hide, !1, (l) => (l.type === gt && !this.$el.contains(l.target)) || (l.type === 'keydown' && l.keyCode === P.ESC)),
							$([document, ...qt(this.$el)], 'scroll', n, {
								passive: !0
							})
						];
						R(this.tooltip, 'hide', () => a.forEach((l) => l()), {
							self: !0
						});
					}),
	...
	var To = {
		...
		events: [
			...
			},
			{
				...
				self: !0,
				handler() {
					((J = this), this.tracker.init(), p(this.targetEl, 'aria-expanded', !0));
		...
		methods: {
			show(t = this.targetEl, e = !0) {
				if ((this.isToggled() && t && this.targetEl && t !== this.targetEl && this.hide(!1, !1), (this.targetEl = t), this.clearTimers(), !this.isActive())) {
					...
					(this.container && E(this.$el) !== this.container && H(this.container, this.$el), (this.showTimer = setTimeout(() => this.toggleElement(this.$el, !0), (e && this.delayShow) || 0)));
				}
			},
			hide(t = !0, e = !0) {
			...
			position() {
				(O(this.$el, 'uk-drop-stack'), c(this.$el, this._style), (this.$el.hidden = !0));
				...
				((this.$el.hidden = !1), c(this.$el, 'maxWidth', ''), this.$el.offsetWidth > s && b(this.$el, 'uk-drop-stack'), c(this.$el, 'maxWidth', s), this.positionAt(this.$el, this.target, this.boundary));
				for (const [n, [o, r, a, l]] of i)
					if (this.axis === o && m([o, !0], this.stretch)) {
						const h = Math.abs(this.getPositionOffset(this.$el)),
							...
							f = S(this.$el);
						(c(this.$el, {
							[r]: (u[a] > f[a] ? u[this.inset ? l : a] - Math.max(S(this.boundary[n])[a], t[n][a] + e) : Math.min(S(this.boundary[n])[l], t[n][l] - e) - u[this.inset ? a : l]) - h,
							[`overflow-${o}`]: 'auto'
						}),
	...
	function Eo(t, e = () => t.$emit()) {
		return $([document, ...qt(t.$el)], 'scroll', e, {
			passive: !0
		});
	}
		...
		return $(document, gt, ({ target: e }) => {
			t.$el.contains(e) ||
				R(
					...
					`${At} ${fi} scroll`,
					({ defaultPrevented: i, type: s, target: n }) => {
						!i && s === At && e === n && !(t.targetEl && Ls(e, t.targetEl)) && t.hide(!1);
					},
					!0
				);
		});
	...
	var Wl = {
			...
			computed: {
				...
				target({ target: t }, e) {
					return t && ((t === !0 && E(this.input) === e && this.input.nextElementSibling) || w(t, e));
				}
			},
			update() {
				var t;
				...
				const n = ji(e) ? 'value' : 'textContent',
					o = e[n],
					r = (t = i.files) != null && t[0] ? i.files[0].name : A(i, 'select') && (s = z('option', i).filter((a) => a.selected)[0]) ? s.textContent : i.value;
				o !== r && (e[n] = r);
			},
			events: [
				{
			...
			update: [
				...
				},
				{
					read(t) {
						...
						if (((s = Math.max(0, Q(s))), !(i || s || n) || Oo(e) || e[0].some((y, C) => e.some((D) => D[C] && D[C].offsetWidth !== y.offsetWidth)))) return (t.translates = t.scrollColumns = !1);
						let r = ql(e, o),
							a,
							l;
						...
						let f, d, g;
						return (
							(s || n) && ((f = h.map((y, C) => (n ? u - y + s : s / (C % 2 || 8)))), n || (s = Math.max(...h.map((y, C) => y + f[C] - u))), (d = Q(this.parallaxStart, 'height', this.$el, !0)), (g = Q(this.parallaxEnd, 'height', this.$el, !0))),
							{
								columns: a,
								translates: l,
								scrollColumns: f,
	...
	var Xl = {
			...
			update: {
				read() {
					...
					if (((t = `calc(${r ? '100vh' : `${o}px`}`), this.expand)) {
						...
						if (this.offsetTop)
							if (r) {
								const a = this.offsetTop === !0 ? this.$el : it(this.offsetTop, this.$el),
									l = Rt(a)[0] - Rt(n)[0];
								t += l > 0 && l < o / 2 ? ` - ${l}px` : '';
							} else t += ` - ${c(n, 'paddingTop')}`;
						this.offsetBottom === !0 ? (t += ` - ${x(this.$el.nextElementSibling).height}px`) : vt(this.offsetBottom) ? (t += ` - ${this.offsetBottom}vh`) : this.offsetBottom && Qt(this.offsetBottom, 'px') ? (t += ` - ${k(this.offsetBottom)}px`) : N(this.offsetBottom) && (t += ` - ${x(it(this.offsetBottom, this.$el)).height}px`);
					}
					return (
						(t += `${e ? ` - ${e}px` : ''})`),
						{
		...
		Jl = '<svg width="14" height="14" viewBox="0 0 14 14"><line fill="none" stroke="#000" stroke-width="1.1" x1="1" y1="1" x2="13" y2="13"/><line fill="none" stroke="#000" stroke-width="1.1" x1="13" y1="1" x2="1" y2="13"/></svg>',
		Kl = '<svg width="20" height="20" viewBox="0 0 20 20"><line fill="none" stroke="#000" stroke-width="1.4" x1="1" y1="1" x2="19" y2="19"/><line fill="none" stroke="#000" stroke-width="1.4" x1="19" y1="1" x2="1" y2="19"/></svg>',
		Zl = '<svg width="12" height="12" viewBox="0 0 12 12"><polyline fill="none" stroke="#000" stroke-width="1.1" points="1 3.5 6 8.5 11 3.5"/></svg>',
		Ql = '<svg width="20" height="20" viewBox="0 0 20 20"><rect x="9" y="4" width="1" height="11"/><rect x="4" y="9" width="11" height="1"/></svg>',
		th = '<svg width="14" height="14" viewBox="0 0 14 14"><polyline fill="none" stroke="#000" stroke-width="1.1" points="1 4 7 10 13 4"/></svg>',
		eh = '<svg width="12" height="12" viewBox="0 0 12 12"><polyline fill="none" stroke="#000" stroke-width="1.1" points="1 3.5 6 8.5 11 3.5"/></svg>',
		ih = '<svg width="12" height="12" viewBox="0 0 12 12"><polyline fill="none" stroke="#000" stroke-width="1.1" points="1 3.5 6 8.5 11 3.5"/></svg>',
		sh = '<svg width="20" height="20" viewBox="0 0 20 20"><style>.uk-navbar-toggle-animate svg&gt;[class*=&quot;line-&quot;]{transition:0.2s ease-in-out;transition-property:transform, opacity;transform-origin:center;opacity:1}.uk-navbar-toggle svg&gt;.line-3{opacity:0}.uk-navbar-toggle-animate[aria-expanded=&quot;true&quot;] svg&gt;.line-3{opacity:1}.uk-navbar-toggle-animate[aria-expanded=&quot;true&quot;] svg&gt;.line-2{transform:rotate(45deg)}.uk-navbar-toggle-animate[aria-expanded=&quot;true&quot;] svg&gt;.line-3{transform:rotate(-45deg)}.uk-navbar-toggle-animate[aria-expanded=&quot;true&quot;] svg&gt;.line-1,.uk-navbar-toggle-animate[aria-expanded=&quot;true&quot;] svg&gt;.line-4{opacity:0}.uk-navbar-toggle-animate[aria-expanded=&quot;true&quot;] svg&gt;.line-1{transform:translateY(6px) scaleX(0)}.uk-navbar-toggle-animate[aria-expanded=&quot;true&quot;] svg&gt;.line-4{transform:translateY(-6px) scaleX(0)}</style><rect class="line-1" y="3" width="20" height="2"/><rect class="line-2" y="9" width="20" height="2"/><rect class="line-3" y="9" width="20" height="2"/><rect class="line-4" y="15" width="20" height="2"/></svg>',
		nh = '<svg width="40" height="40" viewBox="0 0 40 40"><rect x="19" y="0" width="1" height="40"/><rect x="0" y="19" width="40" height="1"/></svg>',
		oh = '<svg width="7" height="12" viewBox="0 0 7 12"><polyline fill="none" stroke="#000" stroke-width="1.2" points="1 1 6 6 1 11"/></svg>',
		rh = '<svg width="7" height="12" viewBox="0 0 7 12"><polyline fill="none" stroke="#000" stroke-width="1.2" points="6 1 1 6 6 11"/></svg>',
		ah = '<svg width="20" height="20" viewBox="0 0 20 20"><circle fill="none" stroke="#000" stroke-width="1.1" cx="9" cy="9" r="7"/><path fill="none" stroke="#000" stroke-width="1.1" d="M14,14 L18,18 L14,14 Z"/></svg>',
		lh = '<svg width="40" height="40" viewBox="0 0 40 40"><circle fill="none" stroke="#000" stroke-width="1.8" cx="17.5" cy="17.5" r="16.5"/><line fill="none" stroke="#000" stroke-width="1.8" x1="38" y1="39" x2="29" y2="30"/></svg>',
		hh = '<svg width="24" height="24" viewBox="0 0 24 24"><circle fill="none" stroke="#000" stroke-width="1.1" cx="10.5" cy="10.5" r="9.5"/><line fill="none" stroke="#000" stroke-width="1.1" x1="23" y1="23" x2="17" y2="17"/></svg>',
		ch = '<svg width="25" height="40" viewBox="0 0 25 40"><polyline fill="none" stroke="#000" stroke-width="2" points="4.002,38.547 22.527,20.024 4,1.5"/></svg>',
		uh = '<svg width="14" height="24" viewBox="0 0 14 24"><polyline fill="none" stroke="#000" stroke-width="1.4" points="1.225,23 12.775,12 1.225,1"/></svg>',
		...
		dh = '<svg width="14" height="24" viewBox="0 0 14 24"><polyline fill="none" stroke="#000" stroke-width="1.4" points="12.775,1 1.225,12 12.775,23"/></svg>',
		ph = '<svg width="30" height="30" viewBox="0 0 30 30"><circle fill="none" stroke="#000" cx="15" cy="15" r="14"/></svg>',
		gh = '<svg width="18" height="10" viewBox="0 0 18 10"><polyline fill="none" stroke="#000" stroke-width="1.2" points="1 9 9 1 17 9"/></svg>',
		Ao = {
			args: 'src',
			props: {
				width: Number,
			...
			disconnected() {
				(this.svg.then((t) => {
					this._connected || (Ri(this.$el) && (this.$el.hidden = !1), at(t), (this.svgEl = null));
				}),
					(this.svg = null));
			},
			methods: {
	...
	const _i = {
		...
		wh = {
			...
			beforeConnect() {
				if (((this.icon = I(this.$el, 'uk-search-icon') && Nt(this.$el, '.uk-search-large').length ? 'search-large' : Nt(this.$el, '.uk-search-navbar').length ? 'search-navbar' : this.$props.icon), !kt(this.$el, 'aria-label')))
					if (I(this.$el, 'uk-search-toggle') || I(this.$el, 'uk-navbar-toggle')) {
	...
	function Eh(t) {
		return _i[t] ? (Oi[t] || (Oi[t] = w((_i[Ph(t)] || _i[t]).trim())), Oi[t].cloneNode(!0)) : null;
	}
	...
	function Hh({ modal: t }) {
		((t.dialog = function (i, s) {
			...
			(t.confirm = function (i, s) {
				return e(
					({ i18n: n }) => `<form> <div class="uk-modal-body">${N(i) ? i : Ht(i)}</div> <div class="uk-modal-footer uk-text-right"> <button class="uk-button uk-button-default uk-modal-close" type="button">${n.cancel}</button> <button class="uk-button uk-button-primary" autofocus>${n.ok}</button> </div> </form>`,
					s,
					() => Promise.reject()
				);
			}),
	...
	var Lh = {
		...
		Wh = {
			...
			computed: {
				...
				dropbarOffset: ({ dropbarTransparentMode: t }, e) => (t === 'behind' ? e.offsetHeight : 0)
			},
			watch: {
				items() {
				...
				registerColorListener() {
					const t =
						...
						!z('.uk-drop', this.dropContainer)
							...
							.some((e) => e.isToggled() && (e.inset || this.getTransparentMode(e.$el) === 'behind'));
			...
			update: {
				read() {
					return !this.content || !this.container || !W(this.$el)
						? !1
						: {
								max: Math.max(this.minHeight, tt(this.container) - (x(this.content).height - tt(this.$el)))
							};
				},
				write({ max: t }) {
					c(this.$el, {
	...
	var tc = {
			...
			observe: de({
				...
				handler(t) {
					...
					for (const { target: i, isIntersecting: s } of t) {
						e.has(i) ||
							e.set(i, {
								cls: Y(i, 'uk-scrollspy-class') || this.cls
							});
						const n = e.get(i);
		...
		ic = {
			...
			computed: {
				selTarget: ({ selTarget: t }, e) => (t && w(t, e)) || e
			},
			connected() {
				((this.start = Ho(this.start || this.top)), (this.end = Ho(this.end || this.bottom)), (this.placeholder = w('+ .uk-sticky-placeholder', this.$el) || w('<div class="uk-sticky-placeholder"></div>')), (this.isFixed = !1), this.setActive(!1));
			},
			beforeDisconnect() {
			...
			update: [
				{
					read({ height: t, width: e, margin: i, sticky: s }) {
						...
						const f = this.overflowFlip ? 0 : Math.max(0, t + u - o),
							...
							C = this.end === !1 ? a : Math.min(a, Ds(this.end, this.$el, d + t, !0) - g - u + f);
						return (
							(s = a && !this.showOnUp && y + u === d && C === Math.min(a, Ds('!*', this.$el, 0, !0) - g - u + f) && c(E(this.$el), 'overflowY') === 'visible'),
							{
								start: y,
								end: C,
								offset: u,
				...
				},
				{
					read({ scroll: t = 0, dir: e = 'down', overflow: i, overflowScroll: s = 0, start: n, end: o }) {
						...
						return {
							...
							scroll: r,
							prevScroll: t,
							offsetParentTop: S((this.isFixed ? this.placeholder : this.$el).offsetParent).top,
							overflowScroll: Z(s + Z(r, n, o) - Z(t, n, o), 0, i)
						};
					},
					write(t, e) {
						const i = e.has('scroll'),
							{ initTimestamp: s = 0, dir: n, prevDir: o, scroll: r, prevScroll: a = 0, top: l, start: h, topOffset: u, height: f } = t;
